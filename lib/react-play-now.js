...
import babelrc from '../fixtures/babelrc.json';
...
export default {
  ...
  activate(state) {
    ...
    atom.workspace.addOpener(uri =>  {
      ...
      if(!this.ongoingPreviews[filename]) {
        this.ongoingPreviews[filename] = new ReactPlayNowView(relativizedPath);
      }

      // shim destroy function to remove from cache
      const _destroy = this.ongoingPreviews[filename].destroy.bind(this.ongoingPreviews[filename]);
      this.ongoingPreviews[filename].destroy = () => {
        // prune preview cache
      ...
      return this.ongoingPreviews[filename];
    });
  },
  ...
  start(target) {
    ...
    atom.workspace.open(`react-play-now://@/${filename}`, { split: 'right' }).then(view => {
      ...
      const watchDeps = dependencies => dependencies.forEach(dep => watcher.add(`${dep}.*`));
      ...
      function render() {
        try {
          view.render(createLiveCompileZone(contextPath, compileZone, watchDeps));
        } catch(e) {
