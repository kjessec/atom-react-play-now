...
export default {
  ...
  start(target) {
    ...
    atom.workspace.open(`react-play-now://@/${filename}`, { split: 'right' }).then(view => {
      ...
      const watcher = this.watchers[filename] = this.watchers[filename] || chokidar.watch(filename).on('change', () => {
        const watched = watcher.getWatched();
        Object.keys(watched).forEach(directory => watched[directory].forEach(filepath => delete module._cache[path.resolve(directory, filepath)]));
      ...
      const watchDeps = dependencies => dependencies.forEach(dep => {
        try {
          watcher.add(require.resolve(dep));
        } catch(e) { /* do nothing */ }
      });
